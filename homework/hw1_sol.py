...
for data_rate in [50e9, 100e9]:
    ...
    sdp.simple_eye(signal_ideal, samples_per_symbol * 3, 100, dt, "{}Gbps 2-PAM Signal".format(data_rate / 1e9),
                   linewidth=1.5)
    ...
    for freq_bw in [20e9, 30e9, 40e9, 50e9]:
        ...
        w, H = sp.signal.freqs([freq_bw * (2 * np.pi)], [1, freq_bw * (2 * np.pi)],
                               np.linspace(0, 0.5 * max_w, ir_length * 4))
        ...
        sdp.simple_eye(signal_filtered[samples_per_symbol * 100:], samples_per_symbol * 3, 100, UI / samples_per_symbol,
                       "{}Gbps 2-PAM Signal with {}GHz Cutoff Filter".format(round(data_rate / 1e9),
                                                                             round(freq_bw * 1e-9)))
...
for data_rate in [50e9, 100e9]:
    ...
    # eye diagram of ideal signal
    sdp.simple_eye(signal_ideal, samples_per_symbol * 3, 100, dt, "{}Gbps 4-PAM Signal".format(data_rate / 1e9),
                   linewidth=1.5)
    ...
    for freq_bw in [20e9, 30e9, 40e9, 50e9]:
        ...
        sdp.simple_eye(signal_filtered[samples_per_symbol * 100:], samples_per_symbol * 3, 100, UI / samples_per_symbol,
                       "{}Gbps 4-PAM Signal with {}GHz Cutoff Filter".format(round(data_rate / 1e9),
                                                                             round(freq_bw * 1e-9)))
